...
//二つ空白を見つけると終了
int judge(int dir_y, int dir_x, int i,int numOfNode,int cnt_flag, int kei){      //int kei = 1or2
  ...
  // if(board[y][x] == 2/kei || cnt_flag==2 || (x < 1 || y < 1) || (x > 15 || y > 15))return numOfNode;
	if(board[y][x] == 1*kei)return judge(y,x,i,numOfNode + 1,cnt_flag,kei);
	if(board[y][x] == 0)return judge(y,x,i,numOfNode,cnt_flag + 1,kei);
}

//一つでも空白を見つけると終了
int judge_adj(int dir_y, int dir_x, int i,int numOfNode, int kei){

  int y = dir_y + dy[i];
  int x = dir_x + dx[i];

	if(board[y][x] == 1*kei)return judge_adj(y,x,i,numOfNode + 1,kei);
...
int win_judge(int dir_y, int dir_x, int kei){		//board[dir_y-1][dir_x-1]のジャッジ
  ...
  for (i = 0; i < 8; i++){
    show_num[i] = judge(y,x,i,0,0, kei);
    jud_num[i] = show_num[i] + judge_adj(y,x,(7-i),0, kei);
  }

  //連続したノードを見つける用
  for(i = 0; i < 4; i++){
    jud_5[i] = judge_adj(y,x,i,0, kei) + judge_adj(y,x,(7-i),0, kei);
  }
...
int ban_judge(int dir_y, int dir_x,int kei){		//board[dir_y-1][dir_x-1]のジャッジ
	ban3_cnt = 0, ban4_cnt = 0, ban6_cnt = 0, five_cnt = 0;
  int jud_num[8] = {0,0,0,0,0,0,0,0};
  int show_num[8] = {0,0,0,0,0,0,0,0};    //表示用
  int jud_5[4] = {0,0,0,0};
